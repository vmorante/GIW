...
from bottle import run, request, post,template;
...
def signup_totp():
    ...
    if password == password2 :
        ...
        try:
            ...
            #se puede quitar
            archivo=open("foto"+".jpg", "wb")
            imagen=urllib.urlopen(qrcode)
            while True:
            ...
            return template("mostrarCodigo",title=name,codigo=qrcode,nombre=_id,semilla=seed)
        except:
             return "<p>El alias de usuario ya existe.</p>"
    else :
        return "<p>Las contraseñas no coinciden.</p>"
...
def login_totp():
    ...
    if cursor <> None:
        ...
        if cursor["password"] == passCod :
            seed = cursor["seed"]
            if topt == onetimepass.get_topt(seed, as_string=True):
                if onetimepass.valid_topt(topt):
                    return "<p>Bienvenido " + cursor["name"] + ".</p>"
                else:
                     return "<p>Usuario o contraseña incorrectos.</p>"
            else:
